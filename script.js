...
function removeFinishedItens() {
  const completedItens = document.querySelectorAll('.completed'); // pega todos os elementos da classe completed;
  for (let index = 0; index < completedItens.length; index += 1) { // varre os elementos;
    orderList.removeChild(completedItens[index]); // remove todos os filhos que possuem tal classe;
  }
}
